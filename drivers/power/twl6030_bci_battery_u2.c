...
//	device_power_control("vusb", off);
	...
	DPWR(" device  off in power off charigng mode \n");
...
int get_bat_volt(void)
{
	...
	DPWR("[twl6030] get_bat_volt , fg_voltage_mV :%d\n",p_di->fg_voltage_mV);
...
#if defined(CONFIG_MAX8971_CHARGER)
				DPWR("stop charing in video mode ");
...
//				DCHG("Change charging mode to USB cause of high temp.(over 45 degree)");
   ...
   */
...
#else
	DPWR("[twl6030] max8971_stop_charging 1 \n");
		max8971_stop_charging();
#endif
	...
	if (p_di->factory_control == FACTORY_CHARGER_DISABLE) {
		...
		charging_ic_deactive();
#else
	DPWR("[twl6030] max8971_stop_charging 2\n");
		max8971_stop_charging();
#endif
		recharging_status = RECHARGING_WAIT_UNSET;
		p_di->charge_status = POWER_SUPPLY_STATUS_DISCHARGING;
...
//	DCHG("[twl6030] charging_mode: %d \n, charging_mode");
	...
	if(p_di->charger_source == POWER_SUPPLY_TYPE_MAINS)
	{
		...
		{	
			...
			DCHG("[twl6030] check voltage 1\n");
			if(get_bat_volt() > 4100 ){
				DCHG("[twl6030] check voltage 2\n");
				max8971_start_charging(TA_RECHARING_CURRENT);
			}
			else
				max8971_start_charging(TA_CHARING_CURRENT);
...
//			DCHG(" check 4-1\n");
	...
	{
		DCHG("convert charging current TA to stop in over 45 temperature and 4.0V \n");
		max8971_stop_charging();
  		temp_charging_change_flag=1;
	}
  ...
  */
	...
	case POWER_SUPPLY_PROP_GAUGE_CONTROL:
        ...
        break;
				...
				DCHG("[twl6030] device power control off\n");
				device_control_off();
				/*                                       */
				cancel_delayed_work(&di->twl6030_bci_monitor_work);
				charger_schedule_delayed_work(&di->twl6030_bci_monitor_work, 0);
